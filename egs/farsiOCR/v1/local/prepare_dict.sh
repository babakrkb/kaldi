...
touch data/local/lm/all
rm -r data/local/lm/all
cat data/train/text > data/local/lm/all
cat data/test/text >> data/local/lm/all
cat data/local/lm/all | cut -d' ' -f2- | tr ' ' "\n" | sort -u > data/local/lm/train.vocab
cat data/local/lm/train.vocab | python3 local/make_latin_words.py > data/train/words2latin
cat data/local/lm/all | cut -d' ' -f2- | python3 local/transcript_to_latin.py data/train/words2latin | cut -d' ' -f2- | tr ' ' "\n" | sort | uniq -c | awk '{if ($1 > 50 || length($2) == 3) print $2}' | fgrep -v '~A' > data/local/phonemeset
